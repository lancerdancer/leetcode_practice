...
class Solution:
    def getSkyline(self, buildings: List[List[int]]) -> List[List[int]]:
        ...
        for i, building in enumerate(buildings):
            ...
            edges.append((left, False, h, i))
            edges.append((right, True, h, i))
        edges.sort()
        removed = set()
        heap = []
        result = []
        ...
        for edge in edges:
            idx, is_right, h, building_id = edge

            if not is_right:
                heappush(heap, (-h, building_id))
            else:
                removed.add(building_id)
            ...
            if result[-1][0] == idx:
                result[-1][-1] = max(result[-1][-1], -heap[0][0])
            elif result[-1][-1] != -heap[0][0]:
                result.append([idx, -heap[0][0]])
        return result